	...
	*/
	...
	{ 
		...
		$this->session->set_userdata(array(
			'logistic_user_id'        => $result['admin_id'],
			'logistic_name'           => $result['firstname'].' '.$result['lastname'],
			'logistic_role_id'        => $result['role_id'],
			'logistic_image'          => $result['image'],
            'logistic_email'          => $result['email'],
            'logistic_token'          => $get_token,
			'logistic_login_status'   => TRUE
		));
                
                //Modify Lass_Access Field in Database
                $this->db->set('last_access',date('Y-m-d h:i:sa'))->where('admin_id',$this->session->userdata('logistic_user_id'))->update('admin_user');
