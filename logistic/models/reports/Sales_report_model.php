...
class Sales_report_model extends CI_Model 
{
	...
	*/
	...
	public function getTotalSales($data = array()) {
            $sql = "SELECT SUM(total) AS total FROM `order` WHERE order_status_id > '0'";
		...
		$sql = "SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, COUNT(*) AS `orders`, SUM((SELECT SUM(op.quantity) FROM `order_product` op WHERE op.order_id = o.order_id GROUP BY op.order_id)) AS products, SUM((SELECT SUM(ot.value) FROM `order_total` ot WHERE ot.order_id = o.order_id AND ot.code = 'tax' GROUP BY ot.order_id)) AS tax, SUM(o.total) AS `total` FROM `order` o";
	...
	public function getTotalOrdersByDay() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, HOUR(date_added) AS hour FROM `order` WHERE order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) = DATE(NOW()) GROUP BY HOUR(date_added) ORDER BY date_added ASC");
	...
	public function getTotalOrdersByWeek() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `order` WHERE  order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= DATE(" . $this->db->escape(date('Y-m-d', $date_start)) . ") GROUP BY DAYNAME(date_added)");
	...
	public function getTotalOrdersByMonth() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `order` WHERE order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= " . $this->db->escape(date('Y') . '-' . date('m') . '-1') . " GROUP BY DATE(date_added)");
	...
	public function getTotalOrdersByYear() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `order` WHERE order_status_id IN(" . implode(",", $implode) . ") AND YEAR(date_added) = YEAR(NOW()) GROUP BY MONTH(date_added)");
