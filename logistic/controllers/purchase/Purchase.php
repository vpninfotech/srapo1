...
* @Auther       : Mitesh
* @Date         : 23-12-2016
* @Description  : Finance Purchase Operation
*
*/
...
class Purchase extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->lang->load('purchase/purchase_lang', 'english');
	...
	*/
	...
	{
		// breadcrumbs
		$this->data['add'] 			 = base_url('purchase/purchase/add');
		if($this->session->userdata('role_id')== 1)
		{
			$this->data['delete'] 		= base_url('purchase/purchase/delete');
		}
		else
		{
			$this->data['delete'] 		= base_url('purchase/purchase/softDelete');
		}
		...
		$url = base_url("purchase/purchase/index/$sort_by/$sort_order");
		$total_records = $this->purchase->getTotalPurchase();
		...
		$results = $this->purchase->getPurchases($data);
		...
		foreach ($results as $result) {
			...
			$this->data['records'][] = array(
				...
				'view'           => base_url('purchase/purchase/view'.$url.'/'.$this->commons->encode($result['purchase_id'])),	
				'edit'             	=>base_url('purchase/purchase/edit'.$url.'/'.$this->commons->encode($result['purchase_id']))
			);
		}		
		...
		$admin_theme = $this->common->config('admin_theme');
		$content_page="themes/".$admin_theme."/purchase/purchase_list";
	...
	*/
	...
	{		
		...
		if(($this->input->server('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
				...
				$purchase_data['manufacturer_id']=$manufacturer_id;
	...
	*/
	public function viewForm()
	{
		...
		$this->data['form_action'] = base_url('purchase/purchase/view');		
		$this->data['purchase_id'] = $this->commons->decode($this->uri->segment($count));
		$this->data['purchase_data'] = $this->purchase->viewPurchaseProduct($this->data['purchase_id']);		
		
		$purchase_data = $this->purchase->viewPurchaseProduct($this->data['purchase_id']);
		...
		$invoice_prefix=$purchase_data['invoice_prefix'];
		$this->data['edit']  = base_url('purchase/purchase/edit'.$url.'/'.$this->commons->encode($purchase_id));
		$this->data['invoice'] = base_url('purchase/purchase/invoice/').$this->commons->encode($purchase_id);				
		...
		////////////
		// Products
		/*$purchase_order_data=$this->purchasecart->getPurchaseProductsData($manufacturer_id,$purchase_id) ;
		
		$this->data['purchase_products']=$purchase_order_data;*/
		...
		$admin_theme = $this->common->config('admin_theme');
		$content_page="themes/".$admin_theme."/purchase/purchase_detail_list";
		$this->load->view($content_page,$this->data);
	}
	...
	* @description   : Check purchase id relation for delete
	* @param   		 : void
	* @return        : void
	*
	*/
	...
	{	
		foreach ($this->input->post('selected') as $purchase_id) 
		{
			$purchaseid_exists = $this->purchase->purchaseid_exists($purchase_id);
			
			if($purchaseid_exists > 0) 
			{
				$this->error['warning'] = $this->lang->line('error_default_purchase');
	...
	*/
	public function invoice($purchase_id = "") {
        ...
        {
            $purchases[] = $this->commons->decode($purchase_id);
        }*/
		//$purchase_id= $this->commons->decode($purchase_id);
        //foreach ($purchases as $purchase_id) {
			
            $purchase_info = $this->purchase->viewPurchaseProduct($purchase_id);
            ...
            if ($purchase_info) {
                ...
                foreach ($products as $product) {
                    ...
                    $product_data[] = array(
                        ...
                        'option'   => $option_data,
                        'quantity' => $product['quantity'],
                       //'price'    => $this->currency->format($product['price'], $purchase_info['currency_code'], $purchase_info['currency_value']),
                        //'total'    => $this->currency->format($product['total'] , $purchase_info['currency_code'], $purchase_info['currency_value'])
						'price'    =>$currency_symbol.' '.number_format((float)$product['price'],$currency_decimal_place, '.', ''),
						'total'    => $currency_symbol.' '.number_format((float)$product['total'],$currency_decimal_place, '.', '')
                    );
                }
               ...
               $total_data = array();
                ...
                /*foreach ($totals as $total) 
                {
                    $total_data[] = array(
                        'title' => 'Total',
                        'text'  => $this->currency->format($total['total'], $purchase_info['currency_code'], $purchase_info['currency_value']),
                    );
                }*/
        ...
        //}
            ...
            $admin_theme = $this->common->config('admin_theme');
            $content_page="themes/".$admin_theme."/purchase/purchase_invoice";
