        ...
        foreach ($orders as $order_id) {
            ...
            if ($order_info && $order_info['shipping_code']) {
                ...
                foreach ($products as $product) {
                    ...
                    if ($product_info) {
                        ...
                        foreach ($options as $option) {
							...
							if ($product_option_value_info && $product_option_value_info['weight_prefix']) {
						...
						foreach($product_info as $productinfo)
						{
							$product_data[] = array(
								...
								'mpn'      => $productinfo['mpn'],
								'weight'   => $productinfo['weight']." ".$productinfo['weight_class']
							);
						}
                    }
                }
                ...
                $data['orders'][] = array(
                    'order_id'         => $order_id,
                    'invoice_no'       => $invoice_no,
                    'date_added'       => date($this->common->config('config_date_format'), strtotime($order_info['date_added'])),
                    'store_name'       => $store_name,
                    'store_address'    => nl2br($store_address),
                    'store_email'      => $store_email,
                    'store_telephone'  => $store_telephone,
