...
class Ticket_model extends CI_Model 
{
    ...
    */
    ...
    /**
    * 
    * @function name : GetTicketsDataById()
    * @description   : get tickets records by $roleId, $customerId
    * @access        : public
    * @param1        : $roleId - department_id
    * @param2        : $customerId - customer_id
    * @return        : array The selected tickets
    *
    */
    function GetTicketsDataById($roleId,$customerId)
    {
        $this->db->select('ticket.*,role.role_name as RoleName');
        $this->db->from('ticket');
        $this->db->join('role','ticket.department_id=role.role_id');
        $this->db->where('ticket.department_id',$roleId);
        $this->db->where('ticket.customer_id',$customerId);
        $this->db->order_by('ticket_id','desc');
        $query=$this->db->get();
        return $query->result_array();
    }
    ...
    */
    ...
    {
    	$sql = "SELECT *, CONCAT(firstname, ' ', lastname) AS name FROM customer where customer_id = '".(int)$customer_id."' and is_deleted = 0";	
    ...
    *
    */
    public function getManufacturerById($expense_user_id)
    {
    	$sql = "SELECT *, CONCAT(firstname,' ',lastname) AS name FROM manufacturer WHERE manufacturer_id = '".$expense_user_id."' and is_deleted = 0";
    ...
    */
	...
	/**
    * 
    * @function name : getUserByAdminId()
    * @description   : get user record by admin_id
    * @access        : public
    * @param   	     : int $admin_id The admin id that you want
    * @return        : array The selected users array
    *
    */
    public function getUserByAdminId($admin_id) 
    {
        $this->db->from('admin_user');
    ...
    */
	function reply()
	{
		...
		$this->db->set('added_by_role_id',$this->session->userdata('role_id'));	
    ...
    */
	function edit_ticket()
	{
		...
		$this->db->set('department_id',$this->input->post('user_type'));	
		$this->db->set('customer_id',$this->input->post('user_list'));
