...
class Login_model extends CI_Model	
{
	...
	*/
	...
	{ 
		...
		$this->session->set_userdata(array(
			'finance_user_id'        => $result['admin_id'],
			'finance_name'           => $result['firstname'].' '.$result['lastname'],
			'finance_role_id'        => $result['role_id'],
			'finance_image'          => $result['image'],
            'finance_email'          => $result['email'],
            'finance_token'          => $get_token,
			'finance_login_status'   => TRUE
		));
                
        //Modify Lass_Access Field in Database
        $this->db->set('last_access',date('Y-m-d h:i:sa'))->where('admin_id',$this->session->userdata('finance_user_id'))->update('admin_user');
	...
	*/
	...
	{
		$this->db->from('admin_user');
		$this->db->where("user_id",$this->session->userdata('finance_user_id'));
		$this->db->where('password', md5($this->input->post('password')));
		$query=$this->db->get();
	...
	*/
        ...
        {	
            ...
            $this->session->set_userdata(array(
                    ...
                    'finance_name'           => NULL,
                    'finance_role_id'        => NULL,
                    'finance_image'          => NULL,
                    'finance_email'          => NULL,
                    'finance_token'          => NULL,                               
                    'finance_admin_status'   => FALSE,
                    'finance_login_status'   => FALSE
	...
	*/
    public function forgotPassword() {
		...
		$acc_code = $RandNo.$res['admin_id'];
               ...
               // === Send Email ====
                    $Template = $this->mailer->Tpl_Email('forgot_password',$this->commons->encode($acc_code));
                        $Recipient = $res['email'];
                        $Filter = array(
                            '{{NAME}}' =>$res['firstname'].' '.$res['lastname'],
                            '{{RESET_CODE}}' =>$acc_code,
                            '{{RESET_LINK}}' =>base_url('common/reset_password/'.$res['admin_id'].'/'.$this->commons->encode($acc_code))
                        );
