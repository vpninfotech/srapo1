...
class Profile extends CI_Controller {
    ...
    */
    public function getForm()
    {
        ...
        if (1) 
        {
            $user_info = $this->profile->getUser($this->session->userdata('manufacturer_user_id'));
        } 
        ...
        $this->data['manufacturer_id'] = $this->session->userdata('manufacturer_user_id');
    ...
    public function validateForm() {
        ...
        if ($method=='add' || $this->input->post('password')) 
        {
            ...
            $c_password = array(
                ...
                'errors' => array('matches'=>'Password and password confirmation do not match!')
            );
        }
        $validation = array(
                        array(
                        ...
                        array(
                            'field' => 'telephone',
                            'label' => 'Telephone', 
                            'rules' => 'trim|required|numeric|min_length[3]|max_length[32]|xss_clean', 
                            'errors' => array('required' => '%s must be greater than 3 and less than 32 characters!','min_length'=>'%s must be between 3 and 32 characters!','max_length'=>'%s must be between 3 and 32 characters!')
                        ),
    ...
    */
    ...
    {  
        ...
        {
            if ($user_info && ($this->input->post('manufacturer_id') != $user_info['manufacturer_id'])) 
            { 
        ...
        {
            if ($user_info && ($this->input->post('manufacturer_id') != $this->session->userdata('manufacturer_user_id'))) 
            { 
                $this->form_validation->set_message('email_check', 'Email ID is already in use!');
                return FALSE;
            }
    ...
    */
    ...
    {    
        ...
        {
            if ($this->session->userdata['manufacturer_user_id'] == $user_id) 
            {                   
