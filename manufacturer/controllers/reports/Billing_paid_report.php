...
/**
* 
* @file name    : Billing_paid_report
* @Auther       : Indrajit Kaplatiya
* @Date         : 04-01-2017
* @Description  : Billing paid Report Controller 
*
*/

class Billing_paid_report extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('reports/billing_paid_report_model','billing_paid_report');		
				
		$this->lang->load('reports/billing_paid_report_lang', 'english');
	...
	*/
	...
	private function _init() {
		...
		$this->output->set_common_meta('Billing Paid Report','sarpo','This is srapo Billing paid Report page');
	}
	...
	*/
	...
	{   
            ...
            $this->data['breadcrumbs'][] = array(
               'text' => 'Billing Paid Report',
               'href' => base_url('reports/billing_paid_report'),		 
            );
            
            $this->load->model('system/order_status_model');
            $this->data['order_statuses']  = $this->order_status_model->getOrderStatuses();
            
            $filter_array             = $this->session->userdata('billing_paid_report_filter_array');
            $filter_date_start        = "";
            $filter_date_end          = "";
            ...
            $url = base_url("reports/billing_paid_report/index/$sort_by/$sort_order");
            $total_records = $this->billing_paid_report->getTotalPurchased($data);
            ...
            $results = $this->billing_paid_report->getPurchased($data);
            ...
            $admin_theme = $this->common->config('admin_theme');
            $content_page="themes/".$admin_theme."/reports/billing_paid_report_list";
    ...
    public function search()
    {
        if ($this->input->post('button_filter') !== NULL) 
        {
           if ($this->input->post('filter_date_start') !== NULL) {
            ...
            $this->session->set_userdata('billing_paid_report_filter_array', $filter);
        }
        if ($this->input->post('button_all') !== NULL) 
        {
           $this->session->set_userdata('billing_paid_report_filter_array', array());
        }
        $this->index();           
    }
