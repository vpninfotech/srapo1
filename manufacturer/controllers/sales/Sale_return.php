...
/**
* 
* @file name    : Sale return
* @Auther       : Mitesh
* @Date         : 6-01-2017
* @Description  : Finance Sale return Operation
*
*/
...
class Sale_return extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->lang->load('sales/sales_return_lang', 'english');
	...
	*/
	...
	private function _init() {
		...
		$this->output->set_common_meta('Sale Return','sarpo','This is srapo Sale Return page');
	}
	...
	*/
	...
	{
		// breadcrumbs
		$this->data['add'] 			 = base_url('sales/sale_return/add');
		if($this->session->userdata('role_id')== 1)
		{
			$this->data['delete'] 		= base_url('sales/sale_return/delete');
		}
		else
		{
			$this->data['delete'] 		= base_url('sales/sale_return/softDelete');
		}
		...
		$this->data['breadcrumbs'][] = array(
		   'text' => 'Sale Return',
		   'href' => base_url('sales/sale_return'),
		...
		$url = base_url("sales/sale_return/index/$sort_by/$sort_order");
		$total_records = $this->sales_return->getTotalSalesReturn();
		...
		$results = $this->sales_return->getSalesReturns($data);
		...
		foreach ($results as $result) {
			...
			//set product oprned or not as yes/no
			if($result['opened'] == 1)
			{
				$opened='Yes';
			}
			...
			$this->data['records'][] = array(
				...
				'view'           		=> base_url('sales/sale_return/view'.$url.'/'.$this->commons->encode($result['purchase_return_id'])),	
				'edit'             		=>base_url('sales/sale_return/edit'.$url.'/'.$this->commons->encode($result['purchase_return_id']))
			);
		}
		...
		$admin_theme = $this->common->config('admin_theme');
		$content_page="themes/".$admin_theme."/sales/sales_return_list";
	...
	*/	
	public function add()	{
		...
		if(($this->input->server('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
				...
				redirect('sales/sale_return/index'.$url);
	     }
		 $this->getForm();
	}
	...
	*/
	public function viewForm()
	{
		...
		$this->data['cancel'] 		= base_url('sales/sale_return/index'.$url);
		...
		$this->data['form_action'] 			= base_url('sales/sales_return/view');		
		$this->data['purchase_return_id'] 	= $this->commons->decode($this->uri->segment($count));
		$this->data['purchase_data'] 		= $this->sales_return->viewSalesReturnProduct($this->data['purchase_return_id']);
		$this->data['view_keyword']			= 1;
		$purchase_return_data 				= $this->sales_return->getSalesReturn($this->data['purchase_return_id']);
		
		$this->data['invoice'] 				= base_url('sales/sale_return/invoice/').$this->commons->encode($this->data['purchase_return_id']);		
		...
		$purchase_data = $this->sales->getSale($purchase_id);
		...
		$admin_theme = $this->common->config('admin_theme');
		$content_page="themes/".$admin_theme."/sales/sales_return_detail_list";
		$this->load->view($content_page,$this->data);
	}
	...
	*/
	...
	{		
		...
		{		
			...
			$this->data['form_action'] 			= base_url('sales/sale_return/add'.$url);
			$this->data['purchase_product_id']  = '';
		...
		{		
			...
			$this->data['form_action'] 		    = base_url('sales/sale_return/edit'.$url.'/'.$this->uri->segment($count));
		...
		/*echo "<pre>";
		print_r($this->data['list_purchase_id']);
		echo "</pre>";
		exit;*/	
	...
	*/
	public function getOrderIdList()
	{
		...
		{
			$orders_data=$this->orders->getOrders();
		}*/
		/*echo "<pre>";
		print_r($orders_data);
		echo "</pre>";*/
	...
	*/
	...
	{		
		...
		$products_info=$this->sales_return->getProducts($data);
	...
	*/
	...
	{		
		...
		$productoptions_info=$this->sales_return->getProductOptions();
		
		/*echo "<pre>";
		print_r($productoptions_info);
		echo "</pre>";
		exit;*/
	...
	*/
	public function invoice($purchase_return_id = "") {
        ...
        $purchases = array();
     		...
     		$purchase_return_data = $this->sales_return->getSalesReturn($purchase_return_id);
			...
			$purchase_info = $this->sales->getSale($purchase_id);
			/*echo "<pre>";
            ...
            $admin_theme = $this->common->config('admin_theme');
            $content_page="themes/".$admin_theme."/sales/sales_return_invoice";
