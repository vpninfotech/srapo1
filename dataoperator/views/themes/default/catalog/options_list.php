      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h2 class="box-title col-sm-6"><i class="fa fa-list"></i>Option List</h2>
          </div>
          <!-- /.box-header -->
          <div class="box-body">
          ...
          <input type="checkbox" onClick="$('input[name*=\'selected\']').prop('checked', this.checked);">
                  ...
                  <th class="<?php if ($sort_by == 'name') echo "sort_$sort_order";?>"><a href="<?php echo base_url('catalog/options/index/name/'.(($sort_order == 'ASC' ) ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Option Name</a></th>
                  <th class="<?php if ($sort_by == 'sort_order') echo "sort_$sort_order";?>"><a href="<?php echo base_url('catalog/options/index/sort_order/'.(($sort_order == 'ASC' ) ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Sort Order</a></th>
        ...
        <?php foreach($records as $row){?>
                <tr>
                  <td><?php if (in_array($row['option_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['option_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['option_id']; ?>" />
                    <?php } ?></td>
...
$('#button-filter').click(function(){
  var url = "<?php echo base_url('catalog/options');?>";
