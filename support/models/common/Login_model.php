...
class Login_model extends CI_Model	
{
	...
	*/
	...
	{	
		...
		$this->db->where_in('role_id',array(7));
		...
		$this->session->set_userdata(array(
			'support_user_id'        => $result['admin_id'],
			'support_name'           => $result['firstname'].' '.$result['lastname'],
			'support_role_id'        => $result['role_id'],
			'support_image'          => $result['image'],
                        'support_email'          => $result['email'],
                        'support_token'          => $get_token,
			'support_login_status'   => TRUE,
			'support_session_id'     => session_id()
		));
                
                //Modify Lass_Access Field in Database
                $this->db->set('last_access',date('Y-m-d h:i:sa'))->where('admin_id',$this->session->userdata('support_user_id'))->update('admin_user');
    }
	...
	*/
	...
	{
		$this->db->from('admin_user');
		$this->db->where("user_id",$this->session->userdata('support_user_id'));
		$this->db->where('password', md5($this->input->post('password')));
		$query=$this->db->get();
	...
	*/
        ...
        {	
            $this->session->set_userdata(array(
                    ...
                    'support_email'          => NULL,
                    'support_token'          => NULL,                               
                    'support_admin_status'   => FALSE,
                    'support_login_status'   => FALSE
