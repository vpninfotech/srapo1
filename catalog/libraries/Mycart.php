...
class Mycart {
 ...
 $this->_CI->load->library('weight');
	...
	public function getProducts() 
	{
		...
		foreach ($cart_query->result_array() as $cart) 
		{
			...
			if ($product_query->num_rows() && ($cart['quantity'] > 0)) 
			{
				...
				$name='';
				$is_single =0;
				$cartprice=json_decode($cart['catalog_no']);
				if(!empty($cartprice))
				{
					if(!empty($cartprice->special))
					{
						$price=$cartprice->special;
					}
					else
					{
						$price=$cartprice->price;
					}
					$is_single = $cart['is_single'];
					
					//$product_query->row['catalog_front_product']=1;
				}
				if($is_single==1)
					$name='Full Catalog : '.$cartprice->catalog_no;
					else
				...
				$product_data[] = array(
					...
					'name'            => $name,
					'catalog_no'      => $product_query->row('catalog_no'),
					'catalog_product'  => $product_query->row('catalog_product'),
	...
	public function add($product_id, $quantity = 1, $option = array(),$catalog_no=array(),$single_product=0)
	{
		...
		if (!$query->row('total'))
		{
			$this->_CI->db->query("INSERT cart SET customer_id = '" . (int)$this->_CI->customer->getId() . "', session_id = '" . $this->session_id . "', product_id = '" . (int)$product_id . "',`option` = '" . json_encode($option) . "', quantity = '" . (int)$quantity . "', date_added = NOW(),catalog_no = '" . json_encode($catalog_no) . "',is_single = '" . (int)$single_product . "'");
	...
	public function getWeight() {
		...
		foreach ($this->getProducts() as $product) 
		{
			if ($product['shipping']) {
				/*------ VPN For Weight ------------*/
				$CatalogData=explode(':',$product['name']);
				if(trim($CatalogData[0]) == 'Full Catalog' )
				{
					$query = $this->_CI->db->query("select sum(weight) as weight from product where catalog_no='".$product['catalog_no']."' AND quantity > '0' and status='1'");
					$product['weight']=$query->row('weight');
				}
				/*------ End VPN For Weight ------------*/
