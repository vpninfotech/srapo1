	...
	*/
	...
	public function add_address() 
	{
		...
		if (($this->input->server('REQUEST_METHOD') == 'POST')) 
		{
			...
			{
				$json['error'] = "Warning:Given address not valid!";
			}
			...
			if (!$json) 
			{
				...
				$address_info =$this->address->getAddress((int)$this->input->post('address_id'));

				if($address_info && isset($address_info['address_id']))
				{
					$this->load->model('system/country_model','country');

				$country_info = $this->country->getCountry($address_info['country_id']);
				...
				$zone_info = $this->zone->getZone($address_info['zone_id']);
	...
	public function methods($country_id='') 
	{
