...
class KaFormat {
	...
	public function __construct()
	{
		...
		$this->_CI->load->language('system/import_product_lang');*/
	}
	...
	*/
	public function formatPassedTime($diff) {
 		...
 		$periods = array( //suffixes
	    	'd' => array(86400, $this->lang->line('text_days')),
	   		'h' => array(3600, $this->lang->line('text_hours')),
      		'm' => array(60, $this->lang->line('text_minutes')),
			's' => array(1, $this->lang->line('text_seconds'))
  		);
	...
	*/	
	public function parseWeight($weight) {
		...
		if (preg_match("/([\d\.\,]*)([\D]*)$/", $weight, $matches)) {
			...
			$qry = $this->db->query("SELECT * FROM weight_class_description	WHERE unit = '" . $matches[2] . "'"
			);
	
			if (!empty($qry->row_array())) {
				$pair['weight_class_id'] = $qry['weight_class_id'];
			}
		}*/
	...
	public function parseLength($length) {
		...
		if (preg_match("/(.*)([\D]*)$/U", $length, $matches)) {
			...
			$qry = $this->db->query("SELECT * FROM length_class_description	WHERE unit = '" . $matches[2] . "'"
			);

			if (!empty($qry->row_array())) {
				$pair['length_class_id'] = $qry['length_class_id'];
			}
		}*/
	...
	public function getWeightUnit($class_id) {
		...
		if (empty($weights[$class_id])) {
			$qry = $this->db->query("SELECT * FROM weight_class_description	WHERE weight_class_id = '".$class_id."'");
	...
	public function getLengthUnit($class_id) {
		...
		if (empty($lengths[$class_id])) {
			$qry = $this->db->query("SELECT * FROM length_class_description WHERE length_class_id = '".$class_id."'");

			if (!empty($qry->row_array())) {
				$lengths[$class_id] = $qry->row_array();
			} else {
