		...
		if($this->CI->Common->config('config_email')) 
		{
			...
			if(strtoupper($this->CI->Common->config('config_mail_protocol')) === 'SMTP')
			{
				$mail = new PHPMailer;
		                    ...
		                    $mail->Host = $this->CI->Common->config('config_mail_smtp_hostname');;
		
		                    $mail->Port = $this->CI->Common->config('config_mail_smtp_port');
		
		                    $mail->SMTPSecure = 'tls';
		
		                    $mail->SMTPAuth = true;
		
		                    $mail->Username = $this->CI->Common->config('config_mail_smtp_username');
		
		                    $mail->Password = $this->CI->Common->config('config_mail_smtp_password');
		
		                    $mail->setFrom($this->CI->Common->config('config_email'), $this->CI->Common->config('config_store_name'));
		
		                    $mail->addReplyTo($this->CI->Common->config('config_email'), $this->CI->Common->config('config_store_name'));
		
		                    $mail->addAddress($Recipient);
		
		                    $mail->Subject = $Template['Subject'];
		
		                    $mail->msgHTML($Template['Message']);
		                    if (!$mail->send()) 
		                    {
