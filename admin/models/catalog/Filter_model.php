...
class Filter_model extends CI_Model 
{
	...
	*/
    ...
    {
		$sql = "SELECT * FROM filter WHERE 1=1";

        if (!empty($data['filter_name'])) {
                $sql .= " AND filter_name LIKE '%" . $data['filter_name'] . "%'";
        }
	...
	* @description   	: get filter group records
	* @access 		 	: public
	* @param   		 	: $data in array format
	* @return       	: array The selected filter group array
	*/
	public function getFilterGroups($data = array()) {
		$sql = "SELECT * FROM `filter_group`";
