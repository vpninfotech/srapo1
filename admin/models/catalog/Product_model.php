...
class Product_model extends CI_Model 
{
	...
	public function getProductById($product_id)
        {
            ...
            $query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM url_alias WHERE query = 'product_id=" . (int)$product_id . "') AS seo_url FROM product  WHERE product_id = '" . (int)$product_id . "'");
	...
	*/
	public function addProduct()
	{
        ...
        $this->db->set('catalog_product',$this->input->post('catalog_product')); 
        $this->db->set('manufacture_catalog_no',$this->input->post('manufacturer_catalog_no')); 
        $this->db->set('manufacture_product_code',$this->input->post('manufacturer_product_code')); 
        $this->db->set('manufacture_catalog_name',$this->input->post('manufacurer_catalog_name')); 
		...
		$this->db->insert('product');
                ...
                if ($product_id && $this->input->post('seo_url') !=="" ) 
                {
                    $this->db->query("INSERT INTO url_alias SET query = 'product_id=" . (int)$product_id . "', keyword = '" . $this->input->post('seo_url') . "'");
                }
	...
	*
	*/
	public function getTotalProductByFilterId($filter_id) 
	{
		$getFilterGroupId = $this->db->query("SELECT filter_id FROM filter WHERE filter_group_id = '".(int)$filter_id."'");
		$getFilterGroupData = $getFilterGroupId->result_array();
                if ($getFilterGroupId->num_rows() > 0) {
                    foreach ($getFilterGroupData as $filterId) { 
                            $sql = "SELECT COUNT(*) AS total FROM product_filter WHERE filter_id = '".(int)$filterId['filter_id']."'";
                            $query = $this->db->query($sql); 
                    } 
                    return $query->row('total');
                }                  
	...
	*/
	public function getTotalProductsByStockStatusId($stock_status_id) {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total FROM product WHERE weight_class = '" . (int)$weight_class_id . "'");

		return $query->row('total');
	}

	public function getTotalProductsByLengthClassId($length_class_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM product WHERE length_class = '" . (int)$length_class_id . "'");
