...
class Attributes_groups extends CI_Controller {
	...
	{		
		if(($this->input->server('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
                    ...
                    if ($this->input->post('is_deleted') == 1) {
                        $res = $this->validateSoftDelete($this->input->post('attribute_group_id'));
                        if($res==0)
                        {
                            $this->session->set_userdata('error',$this->error['warning']);
                            redirect('catalog/attributes_groups/edit'.$url.'/'.$this->commons->encode($this->input->post('attribute_group_id')));  
                        }
                    }  
	...
	*
	*/
	public function validateSoftDelete($attribute_group_id) 
	{	
            $this->load->model('catalog/attributes_model');
