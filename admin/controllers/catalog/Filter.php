...
private $error = array();
	...
	*/
	...
	private function _init() {
		...
		$this->output->set_common_meta('Admin Filter','sarpo','This is srapo Filter page');
	...
	public function edit($sort_by = 'title', $sort_order = 'ASC', $offset = 0)
	{
		...
		if(($this->input->server('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
                    ...
                    if ($this->input->post('is_deleted') == 1) {
                        $res = $this->validateSoftDelete($this->input->post('filter_group_id'));
                        if($res==0)
                        { 
                            $this->session->set_userdata('error',$this->error['warning']);
                            redirect('catalog/filter/edit'.$url.'/'.$this->commons->encode($this->input->post('filter_group_id')));  
                        }
                    }

                    $this->filter->editFilter();
                    $this->session->set_userdata('success',$this->lang->line('text_success'));
                    redirect('catalog/filter/index'.$url);
	...
	*
	*/
	public function validateSoftDelete($filter_id) 
	{
		$this->load->model('catalog/category_model');
		$this->load->model('catalog/product_model');
