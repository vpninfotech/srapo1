...
* 
* @file name    : Category
* @Auther       : Mitesh/Indrajit
* @Date         : 11-11-2016-09/12/2016
...
class Category extends CI_Controller {
	...
	{		
		if(($this->input->server('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
				...
				if ($this->input->post('is_deleted') == 1) {
                                $res = $this->validateSoftDelete($this->input->post('category_id'));
                                if($res==0)
                                {
                                    $this->session->set_userdata('error',$this->error['warning']);
                                    redirect('catalog/category/edit'.$url.'/'.$this->commons->encode($this->input->post('category_id')));  
                                }
                    }    
	...
	*
	*/
	public function validateSoftDelete($category_id) 
	{	
	...
	*/
	function check_exists_seo_keyword($str)
	{
	    ...
	    if($this->input->post('category_id') !="")
	    {
	    	$query = 'category_id='.$this->input->post('category_id');
