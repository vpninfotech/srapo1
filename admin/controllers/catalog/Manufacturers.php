...
class Manufacturers extends CI_Controller {
				...
				else
				{
					$url .= '/0';
                                ...
                                if ($this->input->post('is_deleted') == 1) {
                                    $res = $this->validateSoftDelete($this->input->post('manufacturer_id'));
                                    if($res==0)
                                    {
                                        $this->session->set_userdata('error',$this->error['warning']);
                                        redirect('catalog/manufacturers/edit'.$url.'/'.$this->commons->encode($this->input->post('manufacturer_id')));  
                                    }
                                }   
	...
	*/
	public function getForm()
	{
		...
		if (1) 
		{
			...
			$manufacturer_address_info = $this->manufacturer->getManufacturerAddressById($this->commons->decode($this->uri->segment($count)));
		}
	    ...
	    if ($this->input->post('account_name')!==NULL) {
		 ...
		 if ($this->input->post('cst')!==NULL) {
			$this->data['cst'] = $this->input->post('cst');
		} elseif (!empty($manufacturer_info)) {
		 ...
		 if ($this->input->post('upload_pancard')!==NULL) {
			$this->data['upload_pancard'] = $this->input->post('upload_pancard');
		} elseif (!empty($manufacturer_info)) {
			$this->data['upload_pancard'] = $manufacturer_info['upload_pancard'];
		} else {
			$this->data['upload_pancard'] = '';
		}

		 if ($this->input->post('upload_bank_doc')!==NULL) {
			$this->data['upload_bank_doc'] = $this->input->post('upload_bank_doc');
		} elseif (!empty($manufacturer_info)) {
			$this->data['upload_bank_doc'] = $manufacturer_info['upload_bank_doc'];
		} else {
			$this->data['upload_bank_doc'] = '';
		}
	   ...
	   if ($this->input->post('membership_fee')!==NULL) {
		...
		if ($this->input->post('company_logo')!==NULL) {
			$this->data['company_logo'] = $this->input->post('company_logo');
		} elseif (!empty($manufacturer_info)) {
		...
		//address details
		/*if ($this->input->post('address_firstname')!==NULL) {
			$this->data['address_firstname'] = $this->input->post('address_firstname');
		} elseif (!empty($manufacturer_address_info)) {
			
			$this->data['address_firstname'] = $manufacturer_address_info['firstname'];
		} else {
			$this->data['address_firstname'] = '';
		}
		
		if ($this->input->post('address_lastname')!==NULL) {
			$this->data['address_lastname'] = $this->input->post('address_lastname');
		} elseif (!empty($manufacturer_address_info)) {
			
			$this->data['address_lastname'] = $manufacturer_address_info['lastname'];
		} else {
			$this->data['address_lastname'] = '';
		}
		
		if ($this->input->post('address_company_name')!==NULL) {
			$this->data['address_company_name'] = $this->input->post('address_company_name');
		} elseif (!empty($manufacturer_address_info)) {
			
			$this->data['address_company_name'] = $manufacturer_address_info['company'];
		} else {
			$this->data['address_company_name'] = '';
		}*/
		...
		if ($this->input->post('address_1')!==NULL) {
			...
			$this->data['address_1'] = $manufacturer_address_info['address_1'];
		} else {
			$this->data['address_1'] = '';
		}
		/*echo $manufacturer_address_info['address_2'];
		exit;*/
		/*if ($this->input->post('address_2')!==NULL) {
			$this->data['address_2'] = $this->input->post('address_2');
		} elseif (!empty($manufacturer_address_info)) {			
			$this->data['address_2'] = $manufacturer_address_info['address_2'];
		} else {
			$this->data['address_2'] = '';
		}*/
		...
		if ($this->input->post('country')!==NULL) {
			$this->data['address_country'] = $this->input->post('country');
		} elseif (!empty($manufacturer_address_info)) {			
			$this->data['address_country'] = $manufacturer_address_info['country_id'];
		} else {
			$this->data['address_country'] = '';
		}
		
		if ($this->input->post('state')!==NULL) {
			$this->data['state'] = $this->input->post('state');
		} elseif (!empty($manufacturer_address_info)) {			
			$this->data['state'] = $manufacturer_address_info['state_id'];
		} else {
			$this->data['state'] = '';
		}
		...
		//get state list
		//$this->data['states'] = $this->zone->getZonesByCountryId();
					    ...
					    array(
					      ...
					      array(
						   ...
						   array(
					        'field'  => 'telephone',
					        'label'  => 'Telephone', 
					        'rules'  => 'trim|xss_clean|numeric|min_length[3]|max_length[32]', 
						   ...
						   array(
					        ...
					        'field'  => 'gst',
					        'label'  => 'GST', 
					        'rules'  => 'callback_check_file_extension[company_logo]', 
					        'errors' => array('check_file_extension' => '%s  upload valid file!')
					    ),
						       array(
					        'field'  => 'cst',
					        'label'  => 'CST', 
					        ...
					        'errors' => array('check_file_extension' => '%s  upload valid file!')
						         ...
						         array(
					        'field'  => 'upload_pancard',
					        'label'  => 'Upload Pancard', 
					        'rules'  => 'callback_check_file_extension[company_logo]', 
					        'errors' => array('check_file_extension' => '%s  upload valid file!')
					    ),
						           array(
					        'field'  => 'upload_bank_doc',
						   ...
						   array(
					        ...
					        'rules'  => 'trim|xss_clean|min_length[6]|max_length[128]', 
					        'errors' => array('required' => '%s  must be between 6 and 128 characters!','min_length'=>'%s must be between 6 and 128 characters!','max_length'=>'%s must be between 6 and 128 characters!')
					    ),
						      array(
					        'field'  => 'company_logo',
					        'label'  => 'Company Logo', 
	...
	*
	*/
	public function validateSoftDelete($manufacturer_id) 
	{
		$this->load->model('catalog/product_model');	
	...
	public function check_file_extension($field,$controllerName)
	{
		if($_FILES[$controllerName]['name'] !=="")
		{
			$res = $this->filestorage->CheckFileExtension($controllerName,'image_pdf_doc');
			if($res)
			{
				return true;
			}
