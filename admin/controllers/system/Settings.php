	...
	*/
	public function index()	{
		 ...
		 $this->data['templates'][] = basename($directory);
          ...
          //====Code Start: Get all Site themes directory 
            $this->data['catalog_templates'] = array();
            
            $directories = glob(THEME_PATH.'/catalog/themes/*' , GLOB_ONLYDIR);

            foreach ($directories as $directory) {
		 $this->data['catalog_templates'][] = basename($directory);
            }
            //====Code End: Get all Site themes directory
            ...
            if  (($this->input->post('config_template')) !== NULL) {
                $this->data['config_template'] = $this->input->post('catalog_theme');
            } else {
                $this->data['config_template'] = $this->common->config('catalog_theme');
            }
            ...
            if  (($this->input->post('config_bestseller_limit')) !== NULL) {
                $this->data['config_bestseller_limit'] = $this->input->post('config_bestseller_limit');
            } else {
                $this->data['config_bestseller_limit'] = $this->common->config('config_bestseller_limit');
            }
			
			if  (($this->input->post('config_featured_limit')) !== NULL) {
                $this->data['config_featured_limit'] = $this->input->post('config_featured_limit');
            } else {
                $this->data['config_featured_limit'] = $this->common->config('config_featured_limit');
            }
			
			if  (($this->input->post('config_latest_limit')) !== NULL) {
                $this->data['config_latest_limit'] = $this->input->post('config_latest_limit');
            } else {
                $this->data['config_latest_limit'] = $this->common->config('config_latest_limit');
            }
			
			if  (($this->input->post('config_special_limit')) !== NULL) {
                $this->data['config_special_limit'] = $this->input->post('config_special_limit');
            } else {
                $this->data['config_special_limit'] = $this->common->config('config_special_limit');
            }
            ...
            if  (($this->input->post('config_tax')) !== NULL) {
                $this->data['config_tax'] = $this->input->post('config_tax');
            } else {
                $this->data['config_tax'] = $this->common->config('config_tax');
            }
            ...
            $this->data['customer_group_list'] = $this->customer_groups->getCustomerGroups();

             if  (($this->input->post('config_customer_price')) !== NULL) {
                $this->data['config_customer_price'] = $this->input->post('config_customer_price');
            } else {
                $this->data['config_customer_price'] = $this->common->config('config_customer_price');
            }
           

            if  (($this->input->post('config_customer_group_id')) !== NULL) {
                $this->data['config_customer_group_id'] = $this->input->post('config_customer_group_id');
            } else {
                $this->data['config_customer_group_id'] = $this->common->config('config_customer_group_id');
            }
            ...
            if  (($this->input->post('config_processing_status')) !== NULL) {
                $this->data['config_processing_status'] = $this->input->post('config_processing_status');
            } else {
                if($this->common->config('config_processing_status'))
                {
                    $this->data['config_processing_status'] = json_decode($this->common->config('config_processing_status'),TRUE);    
                }
                else
                {
                    $this->data['config_processing_status'] = array();
                }

                
            }

             if  (($this->input->post('config_complete_status')) !== NULL) {
                $this->data['config_complete_status'] = $this->input->post('config_complete_status');
            } else {
                if($this->common->config('config_complete_status'))
                {
                    $this->data['config_complete_status'] = json_decode($this->common->config('config_complete_status'),TRUE);    
                }
                else
                {
                    $this->data['config_complete_status'] = array();
                }

                
            }
            
             if  (($this->input->post('config_display_categories')) !== NULL) {
                $this->data['config_display_categories'] = $this->input->post('config_display_categories');
            } else {
                if($this->common->config('config_display_categories'))
                {
                    $this->data['config_display_categories'] = json_decode($this->common->config('config_display_categories'),TRUE);    
                }
                else
                {
                    $this->data['config_display_categories'] = array();
                }
            ...
            if  (($this->input->post('config_order_mail')) !== NULL) {
             ...
             if  (($this->input->post('config_display_hot_categories')) !== NULL) {
                $this->data['config_display_hot_categories'] = $this->input->post('config_display_hot_categories');
            } else {
                $this->data['config_display_hot_categories'] = $this->common->config('config_display_hot_categories');
            }

             if  (($this->input->post('config_display_hot_sub_categories')) !== NULL) {
                $this->data['config_display_hot_sub_categories'] = $this->input->post('config_display_hot_sub_categories');
            } else {
                $this->data['config_display_hot_sub_categories'] = $this->common->config('config_display_hot_sub_categories');
            }
		...
		$this->data['icon'] = $this->image->resize('no_image.png', 100, 100);
            ...
            $this->data['Categories'] =$this->category->getAllParentCategories();
	...
	*/
	public function validate() {
            $validation = array(
                            ...
                            array(
                                'field'     => 'config_bestseller_limit',
                                'label'     => 'Display bestseller No of Item', 
                                'rules'     => 'trim|required|xss_clean|integer', 
                                'errors'    => array('required' => 'Limit required!')
                            ),
							array(
                                'field'     => 'config_latest_limit',
                                'label'     => 'Display Latest No of Item', 
                                'rules'     => 'trim|required|xss_clean|integer', 
                                'errors'    => array('required' => 'Limit required!')
                            ),
							array(
                                'field'     => 'config_special_limit',
                                'label'     => 'Display Special No of Item', 
                                'rules'     => 'trim|required|xss_clean|integer', 
                                'errors'    => array('required' => 'Limit required!')
                            ),
							array(
                                'field'     => 'config_featured_limit',
                                'label'     => 'Display Feature No of Item', 
                                'rules'     => 'trim|required|xss_clean|integer', 
                                'errors'    => array('required' => 'Limit required!')
                            ),
                            array(
                           ...
                           array(
                            ...
                            array(
                             ...
                             array(
                                'field'     => 'config_display_hot_categories',
                                'label'     => 'Parent Category Max', 
                                'rules'     => 'trim|required|xss_clean|integer', 
                                'errors'    => array('required' => '%s required!')
                            ),
                             array(
                                'field'     => 'config_display_hot_sub_categories',
                                'label'     => 'Sub Category Max', 
                                'rules'     => 'trim|required|xss_clean|integer', 
                                'errors'    => array('required' => '%s required!')
                            ),
