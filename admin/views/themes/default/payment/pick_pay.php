    ...
    <section class="content">
        ...
        <div class="row">
		   ...
		   <div class="form-group">
            ...
            <div class="col-sm-10">
              <select name="pick_pay_order_status" class="form-control">
                <?php foreach ($order_statuses as $order_status) { ?>
                <?php if ($order_status['order_status_id'] == $pick_pay_order_status) { ?>
		    ...
		    <!------- Start : input group ------>
                            <div class="form-group">
                                ...
                                <div class="col-sm-9 col-md-10">
                                    <input type="text" id="filter_name" name="pick_pay_payment_country_id" class="form-control hidden-print" placeholder="Countries">
                                    <div id="payment-country" class="well well-sm" style="height: 150px; overflow: auto; margin-bottom: 0px;">
<?php foreach ($pick_pay_payment_country_id as $payment_country) { ?>
                                            <div id="payment-country<?php echo $payment_country['country_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $payment_country['country_name']; ?>
                                                <input type="hidden" name="pick_pay_payment_country_id[]" value="<?php echo $payment_country['country_id']; ?>" />
                                            </div>
<?php } ?>
                                    </div>
                                </div>
		   ...
		   <div class="form-group">
            ...
            <div class="col-sm-10">
			<select name="pick_pay_status" class="form-control">
                <?php if ($pick_pay_status) { ?>
		   ...
		   <div class="form-group">
            ...
            <div class="col-sm-10">
              <input type="text" name="pick_pay_sort_order" value="<?php echo $pick_pay_sort_order; ?>" placeholder="<?php echo $entry_sort_order; ?>" id="input-sort_order" class="form-control" />
            </div>
          </div>
      </form>
                    </div>
...
<script>
 $('input[name=\'pick_pay_payment_country_id\']').autocomplete({
    'source': function (request, response) {
        $.ajax({
            url: "<?php echo base_url('payment/pick_pay/autocomplete/') ?>",
            dataType: 'json',
            type: 'POST',
    ...
    'select': function (item) {
        $('input[name=\'pick_pay_payment_country_id\']').val('');
        $('#payment_country' + item['value']).remove();

        $('#payment-country').append('<div id="payment_country' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="pick_pay_payment_country_id[]" value="' + item['value'] + '" /></div>');
    }
});
