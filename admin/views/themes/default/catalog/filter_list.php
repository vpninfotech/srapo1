      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h2 class="box-title col-sm-6"><i class="fa fa-list"></i>Filter List</h2>
          </div>
          <!-- /.box-header -->
          <div class="box-body">
          ...
          <input type="checkbox" onClick="$('input[name*=\'selected\']').prop('checked', this.checked);">
                  ...
                  <th class="<?php if ($sort_by == 'filter_group_name') echo "sort_$sort_order";?>"><a href="<?php echo base_url('catalog/filter/index/filter_group_name/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Filter Group Name</a></th>
                  <th class="<?php if ($sort_by == 'sort_order') echo "sort_$sort_order";?>"><a href="<?php echo base_url('catalog/filter/index/sort_order/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Sort Order</a></th>
        ...
        <?php foreach($records as $row){?>
                <tr class="<?php if(($row['is_deleted'] == 1)) { echo "bg-danger"; } ?>">
                  <td><?php if (in_array($row['filter_group_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['filter_group_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['filter_group_id']; ?>" />
                    <?php } ?></td>
                  <td class=""><?php echo $row['filter_group_name']; ?></td>
...
$('#button-filter').click(function(){
  var url = "<?php echo base_url('catalog/filter');?>";
$('#form-filter').prop('action', url);
$('#form-filter').submit();
});
