<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper"> 
    ...
    <section class="content-header">
      <h1>Expense Categories<small></small> </h1>
     <ul class="breadcrumb">
            ...
            <table id="theme-table" class="table table-bordered table-hover">
              <thead>
                <tr>
                  ...
                  <th class="<?php if ($sort_by == 'expense_category_name') echo "sort_$sort_order";?>"><a href="<?php echo base_url('expense/expense_category/index/expense_category_name/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Expense Category Name</a></th>
                  <th class="<?php if ($sort_by == 'status') echo "sort_$sort_order";?>"><a href="<?php echo base_url('expense/expense_category/index/status/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Status</a></th>              
			  ...
			  <?php foreach($records as $row){?>
                  ...
                  <td><?php if (in_array($row['expense_category_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['expense_category_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['expense_category_id']; ?>" />
                    <?php } ?></td>
                  <td class=""><?php echo $row['expense_category_name']; ?></td>
