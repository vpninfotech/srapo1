...
<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper"> 
    ...
    <section class="content-header">
      <h1>Return Reasons<small></small> </h1>
       <ul class="breadcrumb">
              ...
              <thead>
                <tr>
                  <th class="col-xs-1">
                    ...
                    <th class="<?php if ($sort_by == 'return_reason_name') echo "sort_$sort_order";?>"><a href="<?php echo base_url('system/return_reason/index/return_reason_name/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Return Reason Name</a></th>
                ...
                <?php if(isset($records)) { ?>	 
                    <?php foreach($records as $row){?>
                        <tr class="<?php if(($row['is_deleted'] == 1)) { echo "bg-danger"; } ?>">
                            <td><?php if (in_array($row['return_reason_id'], $selected)) { ?>
                                <input type="checkbox" name="selected[]" value="<?php echo $row['return_reason_id']; ?>" checked="checked" />
                            <?php } else { ?>
                                <input type="checkbox" name="selected[]" value="<?php echo $row['return_reason_id']; ?>" />
                            <?php } ?></td>
                            <td class=""><?php echo $row['return_reason_name']; ?></td>
                            <td class="text-right">
