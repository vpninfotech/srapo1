 ...
 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper"> 
    ...
    <section class="content-header">
      <h1>Return statuses<small></small> </h1>
     <ul class="breadcrumb">
            ...
            <table id="theme-table" class="table table-bordered table-hover">
              <thead>
                <tr>
                  ...
                  <th class="<?php if ($sort_by == 'return_status_name') echo "sort_$sort_order";?>"><a href="<?php echo base_url('system/Return_status/index/return_status_name/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Return Status Name</a></th>
			  ...
			  <?php foreach($records as $row){?>
                  ...
                  <td><?php if (in_array($row['return_status_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['return_status_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['return_status_id']; ?>" />
                    <?php } ?></td>
                  <td class=""><?php echo $row['return_status_name']; ?></td>
                  <td class="text-right">
