  ...
  <div class="content-wrapper"> 
    ...
    <section class="content-header">
      <h1>Stock Statuses<small></small> </h1>
       <ul class="breadcrumb">
      ...
      <div class="col-xs-12">
              ...
              <thead>
                <tr>
                  <th class="col-xs-1">
                    ...
                    <th class="<?php if ($sort_by == 'stock_status_name') echo "sort_$sort_order";?>"><a href="<?php echo base_url('system/stock_status/index/stock_status_name/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Stock Status Name</a></th>
		...
		<?php if(isset($records)) { ?>	 
                <?php foreach($records as $row){?>
                  ...
                  <td><?php if (in_array($row['stock_status_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['stock_status_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['stock_status_id']; ?>" />
                    <?php } ?></td>
                  <td class=""><?php echo $row['stock_status_name']; ?></td>
                  <td class="text-right">
