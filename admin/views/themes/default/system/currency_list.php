    ...
    <section class="content-header">
     ...
     <ul class="breadcrumb">
      ...
      <div class="pull-right">
        <a href="<?php echo $refresh;?>" class="btn btn-warning"><i class="fa fa-refresh"></i></a>
            ...
            <table id="theme-table" class="table table-bordered table-hover">
              <thead>
                <tr>
                  ...
                  <th class="<?php if ($sort_by == 'title') echo "sort_$sort_order";?>"><a href="<?php echo base_url('system/currency/index/title/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Currency Title</a></th>
                  <th class="<?php if ($sort_by == 'code') echo "sort_$sort_order";?>"><a href="<?php echo base_url('system/currency/index/code/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Code</a></th>
                  <th class="<?php if ($sort_by == 'value') echo "sort_$sort_order "; ?>text-right"><a href="<?php echo base_url('system/currency/index/value/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Value</a></th>
                  <th class="<?php if ($sort_by == 'date_modified') echo "sort_$sort_order "; ?>text-right"><a href="<?php echo base_url('system/currency/index/date_modified/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Last Updated</a></th>
			  ...
			  <?php foreach($records as $row){?>
                  ...
                  <td><?php if (in_array($row['currency_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['currency_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['currency_id']; ?>" />
                    <?php } ?></td>
