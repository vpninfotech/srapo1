<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper"> 
    ...
    <section class="content-header">
      <h1>Return Actions<small></small> </h1>
       <ul class="breadcrumb">
    ...
    <?php } ?>
      ...
      <div class="col-xs-12">
        <div class="box">
          ...
          <!-- /.box-header -->
          <div class="box-body">
		   <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form-return_actions">
		   <input type="hidden" name="token" id="token" value="<?php echo $this->session->userdata('token'); ?>" />
            <table id="theme-table" class="table table-bordered table-hover">
                ...
                <tr>
                  <th class="col-xs-1">
					<input type="checkbox" onClick="$('input[name*=\'selected\']').prop('checked', this.checked);">
                          ...
                          <th class="<?php if ($sort_by == 'return_action_name') echo "sort_$sort_order";?>"><a href="<?php echo base_url('system/return_actions/index/return_action_name/'.(($sort_order == 'ASC') ? 'DESC' : 'ASC').'/'.$this->uri->segment(6)); ?>">Return Action Name</a></th>
			 ...
			 <?php foreach($records as $row){?>
                  ...
                  <td><?php if (in_array($row['return_action_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['return_action_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $row['return_action_id']; ?>" />
                    <?php } ?></td>
                  <td class=""><?php echo $row['return_action_name']; ?></td>
				 <td class="text-right">
